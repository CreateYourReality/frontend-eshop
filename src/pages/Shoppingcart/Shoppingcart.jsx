...
const Shoppingcart = () => {
	...
	const getSortType = sortType => {
		switch (sortType) {
			...
			case "Highest Price":
				...
				break;
			case "Highest rating":
				return sortRatingHigh;
				break;
			case "Lowest rating":
				return sortRatingLow;
				break;
			default:
				return;
				break;
	...
	const filterByText = a => {
		return a.product.id.toString().toLowerCase().includes(text.toLowerCase())
			? a
			: a.product.title.toLowerCase().includes(text.toLowerCase())
			? a
			: a.product.brand.toLowerCase().includes(text.toLowerCase())
			? a
			: null;
	};
					...
					shoppingcart.length === 0 ? (
						...
						<div className='productList'>
							{shoppingcart
								...
								.filter(text.length !== 0 ? filterByText : a => a)
								.map((products, index) => (
